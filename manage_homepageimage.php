...
if(isset($_POST['submit'])){
	...
	if($_GET['id']==0){
		if($_FILES['image']['type']!='image/png' && $_FILES['image']['type']!='image/jpg' && $_FILES['image']['type']!='image/jpeg' && $_FILES['image']['type']!='audio/mpeg' && $_FILES['image']['type']!='video/mpeg' && $_FILES['image']['type']!='audio/webm' && $_FILES['image']['type']!='video/webm' && $_FILES['image']['type']!='audio/mp4' && $_FILES['image']['type']!='video/mp4' && $_FILES['image']['type']!='audio/mp3' && $_FILES['image']['type']!='video/Ogg' && $_FILES['image']['type']!='audio/WAV' && $_FILES['image']['type']!='video/WAV' && $_FILES['image']['type']!='application/pdf' && $_FILES['image']['type']!='image/doc'){ 
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				...
				move_uploaded_file($_FILES['image']['tmp_name'],   $targetFilePath.$image);
							...
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									...
									<textarea name="minister" placeholder="Enter minister's name" class="form-control"><?php echo $meta_description?></textarea>
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Date</label>
									<input type="date" name="date" placeholder="Enter posting date" class="form-control" required value="">
								</div>
								...
								<div class="form-group">
									...
									<textarea name="description" placeholder="Enter resource detailed description" class="form-control" required><?php echo $description?></textarea>
								</div>
