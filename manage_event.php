	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['file']['name']!=''){
				...
				$update_sql="update event set name='$name',location='$location',date='$date', time='$time', ,short_desc='$short_desc', detailed_desc='$detailed_desc', file='$file' where id='$id'";
			}else{
				$update_sql="update event set name='$name', location='$location',date='$date', time='$time', short_desc='$short_desc', detailed_desc='$detailed_desc', file='$file'  where id='$id'";
			}
			...
			$file=$_FILES['file']['name'];
			move_uploaded_file($_FILES['file']['tmp_name'],   $targetFilePath.$file);
			mysqli_query($con,"insert into event (name,location,date, time, short_desc, detailed_desc,status,file) values('$name','$location','$date', '$time', '$short_desc', '$detailed_desc',1,'$file')");
		}
		header('location:event.php');
		die();
	}
                        ...
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
								...
								<div class="form-group">
									...
									<input type="date" name="date" placeholder="Enter event date" class="form-control" required value="<?php echo $date?>">
								</div>

								<div class="form-group">
									<label for="categories" class=" form-control-label">Time</label>
									<input type="time" name="time" placeholder="Enter event time" class="form-control" required value="<?php echo $time?>">
								</div>

								<div class="form-group">
									<label for="categories" class=" form-control-label">Short Description</label>
									<textarea name="short_desc" placeholder="Enter event short description" class="form-control" required><?php echo $short_desc?></textarea>
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Detailed Description</label>
									<textarea name="detailed_desc" placeholder="Enter event detailed description" class="form-control" required><?php echo $detailed_desc?></textarea>
								</div>
